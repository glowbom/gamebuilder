...
namespace MoralisUnity.Platform.Utilities
{
    ...
    public class TaskQueue
    {
        ...
        private UniTask GetTaskToAwait(CancellationToken cancellationToken)
        {
            lock (Mutex)
            {
                return (Tail is { } ? Tail : UniTask.FromResult(true));//ask => { }, cancellationToken);
            }
        }
        ...
        public UniTask<T> Enqueue<T>(Func<UniTask, T> taskStart, CancellationToken cancellationToken = default) //where T : UniTask
        {
            UniTask oldTail;
            UniTask<T> task;
            ...
            lock (Mutex)
            {
                oldTail = Tail is { } ? Tail : UniTask.FromResult(true);
                ...
                T result = taskStart(GetTaskToAwait(cancellationToken));

                task = UniTask.FromResult<T>(result);
                ...
                // out of order.
                Tail = UniTask.WhenAll(oldTail, task);
            }
